...
# MAGIC %sql
# MAGIC drop table if exists .ccu030_temp; 
# MAGIC create table if not exists .ccu030_temp as 
# MAGIC select *, 
...
# MAGIC from .ccu030_20221015_1600_patient_skinny_record_enhanced21_fre
...
df = spark.table(f'.ccu030_temp')

# COMMAND ----------

# select columns
df = df.select('id', 'non_covid_death_20', 'age_on_1_1_20', 'female', 'asian', 'black', 'mixed', 'other_ethnicity', 'deci_imd', 'ltc_count', 'medcount', 'count')

# COMMAND ----------

# replace nulls with zeros on some columns
df = df.fillna(value=0,subset=['id', 'non_covid_death_20', 'asian', 'black', 'mixed', 'other_ethnicity', 'ltc_count', 'medcount'])
...
# but first drop NAs
pdf = pdf.dropna()
pdf_GPop = pdf[pdf["id"] == 0]
# weight_GPop = pdf_GPop["count"].copy()
X_GPop = pdf_GPop.drop(["non_covid_death_20", "count"], axis=1)
y_GPop = pdf_GPop["non_covid_death_20"].copy()

# also for ID records only
pdf_id = pdf[pdf["id"] == 1]
# weight_id = pdf_id["count"].copy()
...
# df.show()
df.write.mode("overwrite").saveAsTable(".ccu030_temp")
