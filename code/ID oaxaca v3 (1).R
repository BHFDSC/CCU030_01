...
library(tidyverse)
# create a reverse of id 
df4$id_rev = with(df4, ifelse(id == 1, 0, 1))
...
# table(df4$id, useNA = 'always')
# table(df4$id_rev, useNA = 'always')


library(oaxaca)
tempdf = df4 %>% filter(age_at_covid >= 18 & age_at_covid <= 100)

results = oaxaca(formula = severe ~ age_at_covid + female + black + asian + mixed + other_ethnicity + deci_imd + astrazeneca + pfizer + moderna + ltc_count2 + medcount2 | id_rev | black + asian + mixed + other_ethnicity, data = tempdf, R= NULL, reg.fun = glm, family='binomial')
# !!! oaxaca cannot take 2+ categorical variables after the | id | part of the formula
...
png("~/oaxaca/graphs/test/oaxaca_id_3fold.png")
```
